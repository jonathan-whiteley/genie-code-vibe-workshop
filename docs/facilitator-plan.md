...
| **Module 6 Feature A (Company News) 403s or returns empty** | The app must call `web_search_mcp` as its **service principal** (`lib/deps.py` `workspace_client`); the forwarded user token lacks MCP scope and 403s. Grant the app SP access on the **Connection**: `/explore/connections/web_search_mcp` → Permissions, then **redeploy** (a running worker won't pick up a new grant). Note: do not test `DatabricksMCPClient` from a notebook (it conflicts with the notebook's `typing_extensions`); test in the deployed app. Other traps (router-vs-inline, `:param` regex on URLs, markdown fences, `$`/`chr(36)`) and the working code are in [`docs/patterns/mcp-company-news-pattern.md`](patterns/mcp-company-news-pattern.md). |
